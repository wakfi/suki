    ...
    try {
      ...
      evaled = this.client.token
        ? evaled.replace(
            ...
            `${Buffer.from(this.client.user?.id || "").toString(
              "base64"
            )}.${this.genString(7)}.${this.genString(27)}`
          )
        : evaled;
      ...
      if (evaled.length < 1950)
        ...
        writeFile(
          `${__dirname}/../../../files/EVAL-${interaction.guild?.id}.js`,
          evaled,
          (err: any) => {
      ...
      Math.floor(Math.random() * 10) === 0
        ? embed.setImage("https://i.imgur.com/YHb5kNm.gif")
        : null;
      embed.setColor("RED");
    }
