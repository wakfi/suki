...
export default class Stats extends ApplicationCommand {
  ...
  public async run(interaction: CommandInteraction) {
    const shardData: string[] = this.client.ws.shards.map(
      (shard) =>
        `${selectEmoji(shard.ping)} Shard ID: ${shard.id} \`${
          shard.ping
        } ms\`\n`
    );
    const { data } = await axios.get<any>(`${discordApi}/gateway/bot`, {
      headers: { authorization: `Bot ${process.env.TOKEN}` },
    });

    const amtMessages = await prisma.messageLog.count();
    const amtOfInfractions = await prisma.infraction.count();
    const querySize = await prisma.$queryRaw<
      { pg_size_pretty: string }[]
    >`SELECT pg_size_pretty(pg_database_size('bulbbot'));`;

    const desc: string[] = [
      `Guild Count: \`${numberWithCommas(this.client.guilds.cache.size)}\``,
      `Users: \`${numberWithCommas(
        this.client.guilds.cache.reduce(
          (a: any, g: { memberCount: any }) => a + g.memberCount,
          0
        )
      )}\``,
      `\n**Cache**`,
    ...
    const embed = new MessageEmbed()
      ...
      .addField(
        "Shard Data",
        `Recommended number of shards: \`${data.shards}\`\n${shardData.join(
          ""
        )}`,
        true
      )
      .addField(
        ...
        `Total: \`${data.session_start_limit.total}\`\nRemaining: \`${
          ...
          Math.floor(Date.now() / 1000) +
            data.session_start_limit.reset_after / 1000
        )}>`,
        true
      )
      .setTimestamp();
