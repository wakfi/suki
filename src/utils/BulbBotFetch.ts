...
export default class BulbFetch {
  ...
  public async getGuildMember(
    members: GuildMemberManager | undefined,
    userId: Maybe<Snowflake>
  ): Promise<Optional<GuildMember>> {
  ...
  ): Promise<Optional<GuildChannel | ThreadChannel>> {
    return this.bulbfetch(channels, channelId);
  }

  public async getGuild(
    guildId: Snowflake,
    guilds: Maybe<GuildManager> = this.client.guilds
  ): Promise<Optional<Guild>> {
  ...
  public async bulbfetch<K, Holds, R>(
    manager: Maybe<DataManager<K, Holds, R>>,
    id: Maybe<K>
  ): Promise<Optional<Holds>> {
    return bulbfetch(manager, id);
  }
}

export async function bulbfetch<K, Holds, R>(
  manager: Maybe<DataManager<K, Holds, R>>,
  id: Maybe<K>
): Promise<Optional<Holds>> {
  ...
  return (manager.fetch?.(id).catch(() => undefined) ??
    undefined) as Optional<Holds>;
}
